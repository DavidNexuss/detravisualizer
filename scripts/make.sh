...
configure_cmake() {
  ...
  CC=/usr/bin/gcc CXX=/usr/bin/g++ cmake -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -S . -B "$dir" -DCMAKE_BUILD_TYPE="$type" > /dev/null && 
...
git submodule update --init --recursive && echo -e "$GREEN✔ Update correct. $NC"
