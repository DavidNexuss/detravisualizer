...
#  define IMGUI_DEBUG_LOG_ERROR(...)                                                     \
    do {                                                                                 \
      if (g.DebugLogFlags & ImGuiDebugLogFlags_EventError) IMGUI_DEBUG_LOG(__VA_ARGS__); \
      else                                                                               \
...
template <typename T> T    ImClamp(T v, T mn, T mx) { return (v < mn) ? mn : (v > mx) ? mx :
                                                                                        v; }
...
#    define IM_ASSERT_USER_ERROR(_EXPR, _MSG)                                  \
      do {                                                                     \
        if (!(_EXPR) && ImGui::ErrorLog(_MSG)) { IM_ASSERT((_EXPR) && _MSG); } \
